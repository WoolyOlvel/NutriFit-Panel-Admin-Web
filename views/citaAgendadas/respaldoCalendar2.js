...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const elements = {
    ...
    eventModal: document.getElementById("event-modal") ? 
                new bootstrap.Modal(document.getElementById("event-modal")) : null,
  ...
  function fetchUpcomingEvents() {
    ...
    .then(events => {
      ...
      if (events && events.length > 0) {
        ...
        elements.upcomingEventList.innerHTML = '<div class="text-center">No hay citas próximas</div>';
      }
    })
    .catch(error => {
      console.error("Error al cargar próximas citas:", error);
      if (elements.upcomingEventList) {
        elements.upcomingEventList.innerHTML = '<div class="text-center text-danger">Error al cargar las citas</div>';
      }
    });
  }
  ...
  function showEventDetails(event) {
    ...
    fetchEventDetails(eventId)
      .then(data => {
        ...
        const tipoEvento = data.type === "consulta" ? "Consulta" : "Reservación";
  ...
  function setEventFormValues(data) {
    ...
    if (util.elementExists("event-start-date")) {
      util.setElementValue("event-start-date", formatDateForInput(data.fecha_consulta));
    }
    ...
    if (util.elementExists("proxima-consulta")) {
      util.setElementValue("proxima-consulta", formatDateForInput(data.fecha_proximaConsulta));
    }
    ...
    util.setElementText("event-start-date-tag", formatDate(data.fecha_consulta));
    util.setElementText("event-timepicker1-tag", formatTime(data.fecha_consulta));
    util.setElementText("event-timepicker2-tag", "");
    util.setElementText("event-location-tag", data.nombre_consultorio || "En Espera Asignación Por Nutriologo");
    util.setElementText("event-description-tag", data.direccion_consultorio || "En Espera Asignación Por Nutriologo");
  }
  ...
  function saveEvent() {
    ...
    const estadoProximaConsulta = document.getElementById("estado_proximaConsulta")?.value;
    const nombreConsultorio = document.getElementById("event-location")?.value;
    const direccionConsultorio = document.getElementById("event-description")?.value;
    ...
    // Preparar datos
    const eventData = {
      fecha_consulta: fechaConsulta ? new Date(fechaConsulta).toISOString() : null,
      estado_proximaConsulta: getEstadoCode(estadoProximaConsulta),
    ...
    updateEvent(eventId, eventData)
      ...
      .catch(error => {
        ...
        util.showError("No se pudo actualizar el evento");
      });
  ...
  function renderUpcomingEvent(event) {
    ...
    if (endDate) {
      ...
      timeRangeText = startTime !== endTime ? `${startTime} to ${endTime}` : "Full day event";
    }

    const eventClass = getEventClass(event.type || event.estado_proximaConsulta);
      ...
      <div class="card mb-3" data-id="${event.id}">
        <div class="card-body">
          ...
          <p class="text-muted text-truncate-two-lines mb-0">
            ${description}
            ${event.nombre_consultorio ? '<br><i class="fas fa-clinic-medical me-1"></i> ' + event.nombre_consultorio : ""}
          </p>
          ${event.estado_proximaConsulta !== undefined ? 
            `<div class="mt-2">
              <span style="color:black" class="badge ${getStatusClass(event.estado_proximaConsulta)}">
  ...
  function getEventClass(type) {
    ...
    // Si es string (tipo)
    return type === "consulta" ? "primary" : "secondary";
  }
  ...
  window.editEvent = function(element) {
    ...
    if (eventDetails && eventForm) {
      ...
      const hiddenInputs = eventForm.querySelectorAll(".form-control.d-none, .input-group.d-none, .form-select.d-none");
  ...
  window.cancelEdit = function() {
    ...
    if (eventDetails && eventForm) {
      ...
      const visibleInputs = eventForm.querySelectorAll(".form-control:not(.d-none), .input-group:not(.d-none), .form-select:not(.d-none)");
  ...
  if (elements.calendar) {
    ...
    console.error("No se pudo inicializar el calendario - elemento no encontrado");
  }
  ...
  if (elements.upcomingEventList) {
    ...
    console.error("No se pudo cargar la lista de eventos próximos - elemento no encontrado");
  }
});