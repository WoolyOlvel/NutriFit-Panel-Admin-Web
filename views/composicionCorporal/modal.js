...
function guardar_editar(e) {
  e.preventDefault();
  var formData = new FormData($("#ComposicionCorporal_form")[0]);
  $.ajax({
    url: `${BASE_URL}/api/composicion_corporal/guardar_editar`,
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function () {
    ...
    $("#buttons-datatables").DataTable({
      ...
      ajax: {
        url: `${BASE_URL}/api/composicion_corporal/listar`,
        type: "GET",
        data: {},
      ...
      language: {
        ...
        sZeroRecords:
          ...
          '<h5 class="mt-2">Lo Sentimos! No Se Encontró La Composición Corporal</h5>' +
...
function editar(ComposicionCorporal_ID) {
    $.post(`${BASE_URL}/api/composicion_corporal/mostrar`, { ComposicionCorporal_ID: ComposicionCorporal_ID }, function (data) {
        $("#ComposicionCorporal_ID").val(data.ComposicionCorporal_ID);
        $("#nombre").val(data.nombre);
        $("#lblTitulo").text("Editar Composición Corporal");
        $("#modalCategoria").modal("show");
    });
}

function eliminar(ComposicionCorporal_ID) {
    $("#removeNotificationModal").modal("show");
    $("#delete-notification").on("click");
    $("#delete-notification").on("click", function () {
        $("#removeNotificationModal").modal("hide");
        $.post(`${BASE_URL}/api/composicion_corporal/eliminar`, { ComposicionCorporal_ID: ComposicionCorporal_ID }, function (data) {
            Swal.fire({
                title: "¡Eliminado!",
                text: "Composición Corporal Eliminado exitosamente",
                icon: "success",
            });
            $("#buttons-datatables").DataTable().ajax.reload();
...
$(document).on("click", "#btnNuevo", function () {
    $("#ComposicionCorporal_ID").val("");
    $("#nombre").val("");
    $("#lblTitulo").text("Agregar Nueva Composición Corporal");
    $('#ComposicionCorporal_form')[0].reset(); // Limpiar el formulario
    $("#modalCategoria").modal("show");
})
