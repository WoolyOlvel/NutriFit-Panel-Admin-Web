var Compania_ID = $('#Compania_IDx').val();
...
function guardar_editar(e){
    ...
    formData.append('Compania_ID', $('#Compania_IDx').val());
    $.ajax({
        url:"../../controllers/empresa.php?op=guardar_editar",
        type:"POST",
        data:formData,
        contentType:false,
        processData:false,
        ...
        "language": {
            ...
            "sZeroRecords":    '<div class="noresult">' +
                                    '<div class="text-center">' +
                                        ...
                                        '<h5 class="mt-2">Lo Sentimos! No Se Encontró La Empresa</h5>' +
...
function editar(Empresa_ID){
    $.post("../../controllers/empresa.php?op=mostrar",{Empresa_ID:Empresa_ID},function(data){
        data = JSON.parse(data);
        $('#Empresa_ID').val(data.Empresa_ID);
        $('#Empresa_Nombre').val(data.Empresa_Nombre);
        $('#Empresa_Ruc').val(data.Empresa_Ruc);


       
    });
    $('#lblTitulo').html('Editar Empresa');
...
function eliminar(Empresa_ID) {
    ...
    $('#delete-notification').on('click', function() {
        ...
        Swal.fire({
            title: "¡Eliminado!",
            text: "La Empresa Fue Eliminado Exitosamente",
            icon: "success"
        });
        $.post("../../controllers/empresa.php?op=eliminar",{Empresa_ID:Empresa_ID},function(data){
            console.log(data);
        });
...
/* function eliminar(Empresa_ID) {
    Swal.fire({
        ...
        text: "No Podrá Recuperar La Empresa",
        icon: "warning",
...
$(document).on("click","#btnNuevo",function(){
    ...
    $('#Empresa_Nombre').val('');
    $('#Empresa_Ruc').val('');
    $('#lblTitulo').html('Agregar Nuevo NutriDesafio');
