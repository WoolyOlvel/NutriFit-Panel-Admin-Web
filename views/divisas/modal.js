...
function init() {
  $("#Divisa_form").on("submit", function (e) {
...
function guardar_editar(e) {
  e.preventDefault();
  var formData = new FormData($("#Divisa_form")[0]);
  $.ajax({
    url: `${BASE_URL}/api/divisas/guardar_editar`,
    type: "POST",
    data: formData,
    contentType: false,
    processData: false,
...
$(document).ready(function () {
    ...
    $("#buttons-datatables").DataTable({
      ...
      ajax: {
        url: `${BASE_URL}/api/divisas/listar`,
        type: "GET",
        data: {},
      ...
      language: {
        ...
        sZeroRecords:
          ...
          '<h5 class="mt-2">Lo Sentimos! No Se Encontró La Divisa</h5>' +
...
function editar(Divisa_ID) {
    $.post(`${BASE_URL}/api/divisas/mostrar`, { Divisa_ID: Divisa_ID }, function (data) {
        $("#Divisa_ID").val(data.Divisa_ID);
        $("#nombre").val(data.nombre);
        $("#lblTitulo").text("Editar Divisa");
        $("#modalCategoria").modal("show");
    });
}

function eliminar(Divisa_ID) {
    $("#removeNotificationModal").modal("show");
    $("#delete-notification").on("click");
    $("#delete-notification").on("click", function () {
        $("#removeNotificationModal").modal("hide");
        $.post(`${BASE_URL}/api/divisas/eliminar`, { Divisa_ID: Divisa_ID }, function (data) {
            Swal.fire({
                title: "¡Eliminado!",
                text: "Divisa Eliminado exitosamente",
                icon: "success",
            });
            $("#buttons-datatables").DataTable().ajax.reload();
...
$(document).on("click", "#btnNuevo", function () {
    $("#Divisa_ID").val("");
    $("#nombre").val("");
    $("#lblTitulo").text("Agregar Nueva Divisa");
    $('#Divisa_form')[0].reset(); // Limpiar el formulario
    $("#modalCategoria").modal("show");
})
